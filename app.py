...
def prediction():
    ...
    if(len(user)==0):
        print(f"User name is required.")
        return render_template("index.html", message="User name is mandatory, please specify username to see recommendation list.")
    else:
...
def predict_sentiment():
    ...
    if(len(review_text)==0):
        pred_sentiment="Review text is required."
        return render_template("index.html", sentimentmessage=pred_sentiment)
    else:    
        print(review_text)
